...
const parsePath = (filename) => {
    ...
    const filePaths = filename.split('/').filter((p) => !!p && p !== '');
    const startPath = filePaths[0];
    const startFile = FileDb.getFile(startPath);
    if (!startFile) {
        throw new Error('分享列表未找到该文件');
    }
    const filePath = startFile.path;
    const dirname = path.dirname(filePath);
    const fullPath = [dirname, ...filePaths.slice(1)];
    const finalPath = fullPath.join(path.sep);
    return { finalPath, filePaths, startPath };
};
